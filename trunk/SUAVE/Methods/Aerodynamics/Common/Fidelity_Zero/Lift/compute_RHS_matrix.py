...
## @ingroup Methods-Aerodynamics-Common-Fidelity_Zero-Lift
def compute_RHS_matrix(n_sw,n_cw,delta,phi,conditions,geometry,sur_flag,slipstream): #geometry, prop_wake_model,initial_timestep_offset): #def compute_RHS_matrix(n_sw,n_cw,delta,phi,conditions,geometry,sur_flag,slipstream):  
    ...
    initial_timestep_offset = 0
    
    for propulsor in geometry.propulsors:  
        #-------------------------------------------------------------------------------------------------------
        # PROPELLER SLIPSTREAM MODEL
        #-------------------------------------------------------------------------------------------------------         
        if slipstream and (sur_flag==False) and ('propeller' in propulsor.keys()):#prop_wake_model and ('propeller' in propulsor.keys()):   
    ...
    #for propulsor in geometry.propulsors:  
        ...
        ##-------------------------------------------------------------------------------------------------------         
            ...
            #for i in range(num_prop): 
                #vt = np.mean(prop.outputs.tangential_velocity_distribution_2d, axis=1)  # induced velocitied averaged around the azimuth
                #va = np.mean(prop.outputs.axial_velocity_distribution_2d     , axis=1)  # induced velocitied averaged around the azimuth
                #R0 = prop.hub_radius 
