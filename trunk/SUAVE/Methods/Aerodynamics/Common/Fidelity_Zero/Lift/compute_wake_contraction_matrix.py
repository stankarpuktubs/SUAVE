...
def compute_wake_contraction_matrix(i,ts,prop,N,m,nts,X_pts):
    ...
    va                = np.mean(prop.outputs.axial_velocity_distribution, axis=1) #np.mean(prop.outputs.axial_induced_velocity_2d, axis=1)  # induced velocitied averaged around the azimuth
    R0                = prop.hub_radius 
    R_p               = prop.tip_radius  
