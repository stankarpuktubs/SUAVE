...
def generate_propeller_wake_distribution(prop,m,VD,init_timestep_offset):
    ...
    Na           = len( prop.outputs.azimuthal_distribution) #prop.outputs.number_azimuthal_stations
    Nr           = len( prop.outputs.blade_radial_distribution_normalized) #prop.outputs.number_radial_stations
    omega        = prop.outputs.omega                        
    vt           = prop.outputs.tangential_velocity_distribution_2d  #prop.outputs.disc_tangential_induced_velocity         
    va           = prop.outputs.axial_velocity_distribution          #prop.outputs.disc_axial_induced_velocity 
    ...
    for i in range(num_prop): 
        ...
        if all(prop.rotation != None) and (prop.rotation[i] == 1):        
